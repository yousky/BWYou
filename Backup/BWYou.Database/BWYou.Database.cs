...
namespace BWYou.Database
{
    ...
    {
        private static string[] providerInvariantNames = new string[] {"System.Data.SqlClient", 
        ...
        public static DbProviderFactory getDB(DBname setDB)
        {
            ...
            {
                case DBname.MSSQL:
                    return DbProviderFactories.GetFactory(providerInvariantNames[0]);
                case DBname.MSSQL_noInstall:
                    return System.Data.SqlClient.SqlClientFactory.Instance; //설치 안 하고도 dll에서 받아오기 테스트 안 해봤음.. ~_~;;
                case DBname.Oracle:
                    return DbProviderFactories.GetFactory(providerInvariantNames[1]);
                case DBname.Oracle_noInstall:
                    return System.Data.OracleClient.OracleClientFactory.Instance; //설치 안 하고도 dll에서 받아오기 테스트 안 해봤음.. ~_~;;
                case DBname.OleDb:
                    return DbProviderFactories.GetFactory(providerInvariantNames[2]);
                case DBname.OleDb_noInstall:
                    return System.Data.OleDb.OleDbFactory.Instance; //설치 안 하고도 dll에서 받아오기 테스트 안 해봤음.. ~_~;;
                case DBname.Odbc:
                    return DbProviderFactories.GetFactory(providerInvariantNames[3]);
                case DBname.Odbc_noInstall:
                    return System.Data.Odbc.OdbcFactory.Instance; //설치 안 하고도 dll에서 받아오기 테스트 안 해봤음.. ~_~;;
                case DBname.MySQL:
                    return DbProviderFactories.GetFactory(providerInvariantNames[4]);
                case DBname.MySQL_noInstall:
                    return MySql.Data.MySqlClient.MySqlClientFactory.Instance;   //설치 안 하고도 Mysql 기본 dll에서 받아오기
                default:
                    return DbProviderFactories.GetFactory(providerInvariantNames[2]);
            }
        }
    ...
    ///  -            "MySQL" - MySQL용 : MysqlCon5.0.9.0, 6.4.3.0 테스트 성공. Mysql 버젼에 맞는 커넥터 설치 필요
    ///  -            "MySQL_noInstall" - MySQL용 : MysqlCon5.0.9.0 무설치 버젼 테스트 성공.
    /// </summary>
